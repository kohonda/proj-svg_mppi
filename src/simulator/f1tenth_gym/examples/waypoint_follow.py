...
def main():
    ...
    ego_init = np.array([[conf.sx, conf.sy, conf.stheta]])
    other_init = np.array([[1, 1, 0]] * num_obs)
    for i in range(num_obs):
        other_init[i, 0] += 1.0 * i
        other_init[i, 1] += 1.0 * i

    init_state = np.concatenate((ego_init, other_init), axis=0)

    obs, step_reward, done, info = env.reset(init_state)
    env.render()
    ...
    while not done:
        ...
        ego_control = np.array([[steer, speed]])
        others_control = np.array([[0, 0]] * num_obs)
        control_array = np.concatenate((ego_control, others_control), axis=0)
        obs, step_reward, done, info = env.step(control_array)
        laptime += step_reward
