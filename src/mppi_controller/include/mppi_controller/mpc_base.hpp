...
namespace mppi
{
    namespace cpu
    {
        ...
        public:
            ...
            std::pair<std::vector<StateSeq>, std::vector<double>> get_state_seq_candidates(const int &num_samples, const std::vector<double> &weights) const;

            std::pair<StateSeq, XYCovMatrices> get_proposed_distribution() const;

        private:
            std::pair<std::vector<double>, std::vector<double>> calc_sample_costs(const PriorSamplesWithCosts &sampler, const State &global_init_state, const grid_map::GridMap &obstacle_map,
                                                                                  const grid_map::GridMap &reference_map, StateSeqBatch *global_state_seq_candidates, StateSeqBatch *local_state_seq_candidates) const;
            std::pair<std::vector<double>, std::vector<double>> calc_sample_costs(const PriorSamplesWithCosts &sampler, const State &local_init_state, const grid_map::GridMap &obstacle_map, StateSeqBatch *local_state_seq_candidates) const;
            ...
            void predict_state_seq(const ControlSeq &control_seq, const State &global_init_state, const grid_map::GridMap &reference_map, StateSeq *global_state_seq, StateSeq *local_state_seq) const;

            double constant_speed_prediction(const double &current_speed) const;
            double linear_speed_prediction(const double &current_speed, const double &target_speed, const double &prediction_interval, const double &min_accel, const double &max_accel) const;
            ...
            std::pair<double, double> state_cost(const StateSeq &global_state_seq, const StateSeq &local_base_state_seq, const grid_map::GridMap &obstacle_map, const grid_map::GridMap &ref_path_map) const;
