...
namespace mppi
{
    namespace cpu
    {
        ...
        public:
            ...
            std::tuple<StateSeq, double, double, double> get_predictive_seq(const State &initial_state, const ControlSeq &control_input_seq) const override;
        ...
        private:
            ...
            const double alpha_;                    //!< @brief weighting parameter [0, 1], which balances control penalties from previous control sequence and nominal control sequence.
            const double non_biased_sampling_rate_; //!< @brief non-previous-control-input-biased sampling rate [0, 1], Add random noise to control sequence with this rate.
        ...
        private:
            ...
            ControlSeq grad_reverse_kld(const ControlSeq &mean_seq, const ControlSeq &noised_seq, const ControlSeqCovMatrices &inv_covs,
                                        const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs, PriorSamplesWithCosts *sampler) const;

            ControlSeqBatch grad_reverse_kld_batch(const PriorSamplesWithCosts &samples, const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs) const;

            ControlSeqBatch transport_samples(const PriorSamplesWithCosts &samples_with_cost, const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs, const int &num_itr, const double &step_size);

            ControlSeqBatch transport_samples(const PriorSamplesWithCosts &samples, const ControlSeqBatch &grad_kld_batch, const int &num_itr, const double &step_size) const;
