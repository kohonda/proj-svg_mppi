...
namespace mppi
{
    namespace cpu
    {
        ...
        public:
            PriorSamplesWithCosts(const size_t &num_samples, const size_t &prediction_horizon,
                                  const std::array<double, CONTROL_SPACE::dim> &max_control_inputs,
                                  const std::array<double, CONTROL_SPACE::dim> &min_control_inputs,
                                  const double &non_biased_sampling_rate, const int &thread_num, const int &seed = 42);
            ...
            size_t get_prediction_horizon() const
            {
                return prediction_horizon_;
            }

            ControlSeq get_zero_control_seq() const
            {
                return Eigen::MatrixXd::Zero(prediction_horizon_ - 1, CONTROL_SPACE::dim);
            }
            ...
            {
                return std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(num_samples_, Eigen::MatrixXd::Zero(prediction_horizon_ - 1, CONTROL_SPACE::dim));
            }

            StateSeq get_zero_state_seq() const
            {
                return Eigen::MatrixXd::Zero(prediction_horizon_, STATE_SPACE::dim);
            }

            StateSeqBatch get_zero_state_seq_batch() const
            {
                return std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(num_samples_, Eigen::MatrixXd::Zero(prediction_horizon_, STATE_SPACE::dim));
            }

            ControlSeqCovMatrices get_zero_control_seq_cov_matrices() const
            {
                return std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_horizon_ - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            }
            ...
            ControlSeq get_mean() const
            {
                return control_seq_mean_;
            }

            ControlSeqCovMatrices get_cov_matrices() const
            {
                return control_seq_cov_matrices_;
            }

            ControlSeqCovMatrices get_inv_cov_matrices() const
            {
                return control_seq_inv_cov_matrices_;
            }
