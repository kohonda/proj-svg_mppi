...
namespace mppi
{
    namespace cpu
    {
        ...
        // q_terminal_speed_(params.q_terminal_speed),
        {
            ...
            global_state_seq_candidates_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(sample_num, Eigen::MatrixXd::Zero(prediction_step_size_, STATE_SPACE::dim));
            local_state_seq_candidates_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(sample_num, Eigen::MatrixXd::Zero(prediction_step_size_, STATE_SPACE::dim));
        }
        ...
        std::pair<std::vector<StateSeq>, std::vector<double>> MPCBase::get_state_seq_candidates(const int &_num_samples, const std::vector<double> &weights) const
        {
            if (weights.size() == 0)
            {
        ...
        std::pair<std::vector<double>, std::vector<double>> MPCBase::calc_sample_costs(const PriorSamplesWithCosts &sampler,
                                                                                       const State &global_init_state, const grid_map::GridMap &obstacle_map,
                                                                                       const grid_map::GridMap &reference_map, StateSeqBatch *global_state_seq_candidates,
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < sampler.get_num_samples(); i++)
            {
                ...
                predict_state_seq(sampler.noised_control_seq_samples_[i], global_init_state, reference_map, &global_state_seq_candidates->at(i), &local_state_seq_candidates->at(i));

                // Calculate cost
                const auto [cost, collision_cost] = state_cost(global_state_seq_candidates->at(i), local_state_seq_candidates->at(i), obstacle_map, reference_map);
                costs.at(i) = cost;
        ...
        std::pair<std::vector<double>, std::vector<double>> MPCBase::calc_sample_costs(const PriorSamplesWithCosts &sampler, const State &local_init_state, const grid_map::GridMap &obstacle_map, StateSeqBatch *local_state_seq_candidates) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        void MPCBase::predict_state_seq(const ControlSeq &control_seq, const State &global_init_state, const grid_map::GridMap &reference_map, StateSeq *global_state_seq, StateSeq *local_state_seq) const
        {
            // observed state
        ...
        double MPCBase::constant_speed_prediction(const double &current_speed) const
        {
            return current_speed;
        }

        double MPCBase::linear_speed_prediction(const double &current_speed, const double &target_speed, const double &prediction_interval, const double &min_accel, const double &max_accel) const
        {
        ...
        std::pair<double, double> MPCBase::state_cost(const StateSeq &local_state_seq, const grid_map::GridMap &obstacle_map) const
        {
            ...
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
                ...
                {
                    collision_cost = obstacle_map.atPosition(obstacle_layer_name_, grid_map::Position(local_state(STATE_SPACE::x), local_state(STATE_SPACE::y)));
                }
            ...
            {
                collision_cost = obstacle_map.atPosition(obstacle_layer_name_, grid_map::Position(terminal_local_state(STATE_SPACE::x), terminal_local_state(STATE_SPACE::y)));
            }
        ...
        std::pair<double, double> MPCBase::state_cost(const StateSeq &global_state_seq, const StateSeq &local_state_seq, const grid_map::GridMap &local_obstacle_map, const grid_map::GridMap &ref_path_map) const
        {
            // calc cost for each state
            ...
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
                ...
                {
                    diff_ref_path = ref_path_map.atPosition(distance_field_layer_name_, grid_map::Position(global_state(STATE_SPACE::x), global_state(STATE_SPACE::y)));
                    diff_angle = global_state(STATE_SPACE::yaw) - ref_path_map.atPosition(angle_field_layer_name_, grid_map::Position(global_state(STATE_SPACE::x), global_state(STATE_SPACE::y)));
                ...
                {
                    collision_cost = local_obstacle_map.atPosition(obstacle_layer_name_, grid_map::Position(local_state(STATE_SPACE::x), local_state(STATE_SPACE::y)));
                }
            ...
            {
                diff_terminal_ref_path = ref_path_map.atPosition(distance_field_layer_name_, grid_map::Position(global_terminal_state(STATE_SPACE::x), global_terminal_state(STATE_SPACE::y)));
                diff_terminal_angle = global_terminal_state(STATE_SPACE::yaw) - ref_path_map.atPosition(angle_field_layer_name_, grid_map::Position(global_terminal_state(STATE_SPACE::x), global_terminal_state(STATE_SPACE::y)));
            ...
            {
                collision_cost = local_obstacle_map.atPosition(obstacle_layer_name_, grid_map::Position(local_terminal_state(STATE_SPACE::x), local_terminal_state(STATE_SPACE::y)));
            }
        ...
        std::pair<StateSeq, XYCovMatrices> MPCBase::calc_state_distribution(const StateSeqBatch &state_seq_candidates) const
        {
            ...
            XYCovMatrices xy_cov_matrices = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_step_size_, Eigen::MatrixXd::Zero(2, 2));
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < state_seq_candidates.size(); i++)
            {
