...
namespace mppi
{
    namespace cpu
    {
        PriorSamplesWithCosts::PriorSamplesWithCosts(const size_t &num_samples, const size_t &prediction_horizon,
                                                     const std::array<double, CONTROL_SPACE::dim> &max_control_inputs,
                                                     const std::array<double, CONTROL_SPACE::dim> &min_control_inputs,
                                                     const double &non_biased_sampling_rate, const int &thread_num, const int &seed)
            : thread_num_(thread_num), num_samples_(num_samples), prediction_horizon_(prediction_horizon),
              non_biased_sampling_rate_(non_biased_sampling_rate), max_control_inputs_(max_control_inputs),
              min_control_inputs_(min_control_inputs)
        {
            control_seq_mean_ = Eigen::MatrixXd::Zero(prediction_horizon - 1, CONTROL_SPACE::dim);
            control_seq_cov_matrices_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_horizon - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            control_seq_inv_cov_matrices_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_horizon - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            noised_control_seq_samples_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(num_samples, Eigen::MatrixXd::Zero(prediction_horizon - 1, CONTROL_SPACE::dim));
            noise_seq_samples_ = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(num_samples, Eigen::MatrixXd::Zero(prediction_horizon - 1, CONTROL_SPACE::dim));
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < num_samples_; i++)
            {
                ...
                for (size_t j = 0; j < CONTROL_SPACE::dim; j++)
                {
                    ...
                    {
                        noised_control_seq_samples_[i](k, j) = std::clamp(noised_control_seq_samples_[i](k, j), min_control_inputs_[j], max_control_inputs_[j]);
                    }
                }
            }
        }
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        std::vector<double> PriorSamplesWithCosts::get_costs_with_control_term(const double &lambda, const double &alpha, const ControlSeq &nominal_control_seq) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < num_samples_; i++)
            {
                for (size_t j = 0; j < prediction_horizon_ - 1; j++)
                {
                    const double control_term = lambda * (1 - alpha) * (control_seq_mean_.row(j) - nominal_control_seq.row(j)) * control_seq_inv_cov_matrices_[j] * noised_control_seq_samples_[i].row(j).transpose();
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < num_inflated_samples; i++)
            {
                ...
                noised_control_seq_samples_[source_samples.num_samples_ + i] = source_samples.noised_control_seq_samples_[indices[i]] + noise_seq_samples_[source_samples.num_samples_ + i];
                ...
                for (size_t j = 0; j < CONTROL_SPACE::dim; j++)
                {
                    for (size_t k = 0; k < prediction_horizon_ - 1; k++)
                    {
                        noised_control_seq_samples_[source_samples.num_samples_ + i](k, j) = std::clamp(noised_control_seq_samples_[source_samples.num_samples_ + i](k, j), min_control_inputs_[j], max_control_inputs_[j]);
                    }
                }
            }
        }

        void PriorSamplesWithCosts::set_control_seq_mean(const ControlSeq &control_seq_mean)
        {
            control_seq_mean_ = control_seq_mean;
        }
        ...
        void PriorSamplesWithCosts::set_control_seq_cov_matrices(const ControlSeqCovMatrices &control_seq_cov_matrices)
        {
            ...
            for (size_t i = 0; i < prediction_horizon_ - 1; i++)
            {
                control_seq_inv_cov_matrices_[i] = control_seq_cov_matrices_[i].inverse() + eps * Eigen::MatrixXd::Identity(CONTROL_SPACE::dim, CONTROL_SPACE::dim);
            }
        }
