...
namespace mppi
{
    namespace cpu
    {
              ...
              steer_cov_(reverse_mppi_params.steer_cov)
        {
            ...
            inflated_samples_ptr_ = std::make_unique<PriorSamplesWithCosts>(inflated_sample_batch_num, prediction_step_size_,
                                                                            max_control_inputs, min_control_inputs,
                                                                            non_biased_sampling_rate_, thread_num_);
...
#pragma omp parallel for num_threads(thread_num_)
                    ...
                    const std::vector<int> selected_indices = inflated_samples_ptr_->random_sample_choice(prior_samples_ptr_->get_num_samples(), probabilities);
                ...
                collision_num = std::count_if(collision_costs.begin(), collision_costs.end(), [](const double &cost)
                                              { return cost > 0.0; });
        ...
        void ReverseMPPI::set_obstacle_map(const grid_map::GridMap &obstacle_map)
        {
            mpc_base_ptr_->set_obstacle_map(obstacle_map);
        };

        void ReverseMPPI::set_reference_map(const grid_map::GridMap &reference_map)
        {
        ...
        std::tuple<StateSeq, double, double, double> ReverseMPPI::get_predictive_seq(const State &initial_state, const ControlSeq &control_input_seq) const
        {
        ...
        ControlSeq ReverseMPPI::get_control_seq() const
        {
            return prev_control_seq_;
        }

        std::pair<StateSeq, XYCovMatrices> ReverseMPPI::get_proposed_state_distribution() const
        {
        ...
        std::pair<std::vector<double>, std::vector<double>> ReverseMPPI::calc_weights(const PriorSamplesWithCosts &prior_samples_with_costs) const
        {
            ...
            const std::vector<double> costs_with_control_term = prior_samples_with_costs.get_costs_with_control_term(lambda_, alpha_, prior_samples_ptr_->get_zero_control_seq());
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < prior_samples_with_costs.get_num_samples(); i++)
            {
                ...
                const double negative_term = negative_ratio_ * std::exp(1.0 / lambda_ * (costs_with_control_term[i] - min_cost)) / negative_normalization_term;
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        std::pair<ControlSeq, ControlSeqCovMatrices> ReverseMPPI::weighted_mean_and_sigma(const PriorSamplesWithCosts &samples, const std::vector<double> &weights) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        std::pair<ControlSeq, ControlSeqCovMatrices> ReverseMPPI::md_update(const ControlSeq &prior_mean, const ControlSeqCovMatrices &prior_covs,
                                                                            const ControlSeq &weighted_mean, const ControlSeqCovMatrices &weighted_covs, const double step_size) const
        {
            ControlSeq updated_mean = prior_mean * 0.0;
            updated_mean = prior_mean - step_size * (prior_mean - weighted_mean);

            ControlSeqCovMatrices prior_stds = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_step_size_ - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            ControlSeqCovMatrices weighted_stds = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_step_size_ - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
            ...
            ControlSeqCovMatrices tmp = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_step_size_ - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
                tmp[i] = -step_size * (prior_stds[i] - weighted_stds[i]);
            }
            ControlSeqCovMatrices updated_covs = std::vector<Eigen::MatrixXd, Eigen::aligned_allocator<Eigen::MatrixXd>>(prediction_step_size_ - 1, Eigen::MatrixXd::Zero(CONTROL_SPACE::dim, CONTROL_SPACE::dim));
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
                const Eigen::MatrixXd updated_std = 0.5 * (tmp[i] + (tmp[i].transpose() * tmp[i] + 4.0 * prior_stds[i].transpose() * prior_stds[i]).cwiseSqrt());
        ...
        ControlSeq ReverseMPPI::normal_pdf(const ControlSeq &x, const ControlSeq &mean, const ControlSeqCovMatrices &var) const
        {
            ...
            for (size_t i = 0; i < prediction_step_size_ - 1; i++)
            {
                pdf.row(i) = (x.row(i) - mean.row(i)).cwiseProduct((var[i].diagonal().cwiseSqrt().asDiagonal().inverse() * (x.row(i) - mean.row(i)).transpose()).transpose()).array().exp() / std::sqrt(std::pow(2.0 * M_PI, CONTROL_SPACE::dim) * var[i].determinant());
            }
            return pdf;
        }
        ...
        ControlSeqCovMatrices ReverseMPPI::interpolate(const ControlSeqCovMatrices &covs1, const ControlSeqCovMatrices &covs2, const double &ratio) const
        {
