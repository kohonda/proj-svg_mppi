              ...
              is_max_posterior_estimation_(sv_mpc_params.is_max_posterior_estimation)
        {
            ...
            for (size_t i = 0; i < sample_batch_num; i++)
            {
                grad_sampler_ptrs_.emplace_back(std::make_unique<PriorSamplesWithCosts>(sample_num_for_grad_estimation, prediction_step_size_,
                                                                                        max_control_inputs, min_control_inputs, non_biased_sampling_rate_, 1, i));
            }
        }
...
#pragma omp parallel for num_threads(thread_num_)
            ...
            auto has_nan_or_inf = [](const double &x)
            {
            ...
            else
            {
                ...
                prior_samples_ptr_->random_sampling(prior_samples_ptr_->get_zero_control_seq(), prior_samples_ptr_->get_constant_control_seq_cov_matrices({steer_cov_}));
        ...
        std::tuple<StateSeq, double, double, double> SteinVariationalMPC::get_predictive_seq(const State &initial_state, const ControlSeq &control_input_seq) const
        {
        ...
        ControlSeq SteinVariationalMPC::get_control_seq() const
        {
        ...
        // == private functions ==
        ControlSeq SteinVariationalMPC::approx_grad_log_likelihood(const ControlSeq &mean_seq, const ControlSeq &noised_seq, const ControlSeqCovMatrices &inv_covs,
        ...
        ControlSeq SteinVariationalMPC::grad_log_normal_dist(const ControlSeq &sample, const ControlSeq &prior_mean, const ControlSeqCovMatrices &prior_covs) const
        {
        ...
        ControlSeqBatch SteinVariationalMPC::approx_grad_posterior_batch(const PriorSamplesWithCosts &samples, const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        std::pair<ControlSeq, ControlSeqCovMatrices> SteinVariationalMPC::weighted_mean_and_sigma(const PriorSamplesWithCosts &samples, const std::vector<double> &weights) const
        {
