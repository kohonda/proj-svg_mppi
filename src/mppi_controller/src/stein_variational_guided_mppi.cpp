...
namespace mppi
{
    namespace cpu
    {
              ...
              max_steer_cov_(svg_mppi_params.max_steer_cov)
        {
            ...
            prior_samples_ptr_ = std::make_unique<PriorSamplesWithCosts>(sample_batch_num, prediction_step_size_,
                                                                         max_control_inputs, min_control_inputs,
                                                                         non_biased_sampling_rate_, thread_num_);

            guide_samples_ptr_ = std::make_unique<PriorSamplesWithCosts>(guide_sample_num, prediction_step_size_,
                                                                         max_control_inputs, min_control_inputs,
                                                                         non_biased_sampling_rate_, thread_num_);
            ...
            for (size_t i = 0; i < sample_batch_num; i++)
            {
                grad_sampler_ptrs_.emplace_back(std::make_unique<PriorSamplesWithCosts>(sample_num_for_grad_estimation_, prediction_step_size_,
                                                                                        max_control_inputs, min_control_inputs, non_biased_sampling_rate_, thread_num_, i));
            }
        }

        std::pair<ControlSeq, double> SVGuidedMPPI::solve(const State &initial_state)
        {
            // calculate nominal control sequence by SVGD without kernel
            ...
            // Transport nominal particles by SVGD
            ...
            for (int i = 0; i < num_svgd_iteration_; i++)
            {
                ...
                // const ControlSeqBatch phis = phi_batch(*guide_samples_ptr_, grad_log_posterior);
#pragma omp parallel for num_threads(thread_num_)
                ...
                // const std::vector<double> cost_with_control_term = guide_samples_ptr_->get_costs_with_control_term(gaussian_fitting_lambda, 0, prior_samples_ptr_->get_zero_control_seq());
                costs_history.insert(costs_history.end(), costs.begin(), costs.end());
                control_seq_history.insert(control_seq_history.end(), guide_samples_ptr_->noised_control_seq_samples_.begin(), guide_samples_ptr_->noised_control_seq_samples_.end());
            }
            ...
            // TODO: ベストparticleのやつだけを使ってcovを計算するようにリファクタする
            ...
            // const ControlSeqCovMatrices given_covs = prior_samples_ptr_->get_constant_control_seq_cov_matrices({steer_cov_});
            // prior_samples_ptr_->random_sampling(prev_control_seq_, given_covs);
            ...
            const int collision_num = std::count_if(collision_costs.begin(), collision_costs.end(), [](const double &cost)
                                                    { return cost > 0.0; });
        ...
        void SVGuidedMPPI::set_reference_map(const grid_map::GridMap &reference_map)
        {
        ...
        std::tuple<StateSeq, double, double, double> SVGuidedMPPI::get_predictive_seq(const State &initial_state, const ControlSeq &control_input_seq) const
        {
        ...
        // == private functions ==
        ControlSeq SVGuidedMPPI::approx_grad_log_likelihood(const ControlSeq &mean_seq, const ControlSeq &noised_seq, const ControlSeqCovMatrices &inv_covs,
                                                            const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs, PriorSamplesWithCosts *sampler) const
        {
            ...
            // const std::vector<double> costs_with_control_term = sampler->get_costs_with_control_term(grad_lambda_, 0.0, sampler->get_zero_control_seq());
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < sampler->get_num_samples(); i++)
            {
                ...
                {
                    // const double control_term = grad_lambda_ * prev_control_seq_.row(j) * inv_covs[j] * sampler->noised_control_seq_samples_[i].row(j).transpose();
                    const double diff_control_term = grad_lambda_ * (prev_control_seq_.row(j) - sampler->noised_control_seq_samples_[i].row(j)) * inv_covs[j] * (prev_control_seq_.row(j) - sampler->noised_control_seq_samples_[i].row(j)).transpose();
        ...
        ControlSeq SVGuidedMPPI::grad_log_normal_dist(const ControlSeq &sample, const ControlSeq &prior_mean, const ControlSeqCovMatrices &prior_covs) const
        {
        ...
        ControlSeqBatch SVGuidedMPPI::approx_grad_posterior_batch(const PriorSamplesWithCosts &samples, const std::function<std::vector<double>(const PriorSamplesWithCosts &)> &calc_costs) const
        {
            ...
            {
                const ControlSeq grad_log_likelihood = approx_grad_log_likelihood(mean, samples.noised_control_seq_samples_[i], samples.get_inv_cov_matrices(), calc_costs, grad_sampler_ptrs_.at(i).get());
                // const ControlSeq grad_log_prior = grad_log_normal_dist(samples.noised_control_seq_samples_[i], samples.get_mean(), samples.get_inv_cov_matrices());
                // const ControlSeq grad_log_prior = grad_log_normal_dist(samples.noised_control_seq_samples_[i], prev_control_seq_, samples.get_inv_cov_matrices());
        ...
        double SVGuidedMPPI::RBF_kernel(const ControlSeq &seq1, const ControlSeq &seq2, const double &h) const
        {
            const double kernel = (seq1 - seq2).squaredNorm();
            return std::exp(-kernel / h);
        }

        ControlSeq SVGuidedMPPI::grad_RBF_kernel(const ControlSeq &seq, const ControlSeq &seq_const, const double &h) const
        {
        ...
        ControlSeqBatch SVGuidedMPPI::phi_batch(const PriorSamplesWithCosts &samples, const ControlSeqBatch &grad_posterior_batch) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
        ...
        std::pair<ControlSeq, ControlSeqCovMatrices> SVGuidedMPPI::weighted_mean_and_sigma(const PriorSamplesWithCosts &samples, const std::vector<double> &weights) const
        {
...
#pragma omp parallel for num_threads(thread_num_)
            for (size_t i = 0; i < samples.get_num_samples(); i++)
            {
                for (size_t j = 0; j < prediction_step_size_ - 1; j++)
                {
                    sigma[j] += (samples.noised_control_seq_samples_[i].row(j) - mu.row(j)).transpose() * (samples.noised_control_seq_samples_[i].row(j) - mu.row(j));
                }
            }
        ...
        std::vector<double> SVGuidedMPPI::calc_weights(const PriorSamplesWithCosts &prior_samples_with_costs, const ControlSeq &nominal_control_seq) const
        {
            // calculate costs with control term
            const std::vector<double> costs_with_control_term = prior_samples_with_costs.get_costs_with_control_term(lambda_, alpha_, nominal_control_seq);
        ...
        std::pair<double, double> SVGuidedMPPI::gaussian_fitting(const std::vector<double> &x, const std::vector<double> &y) const
        {
            ...
            std::transform(y.begin(), y.end(), y_hat.begin(), [](double y)
                           { return std::max(y, 1e-10); });
